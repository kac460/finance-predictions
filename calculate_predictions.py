...
def print_average_expenses(year, month_num):
    ...
    print("-----------\n")
    print(
        f"AVERAGE MONTHLY EXPENSE: {sum([total_expenses_in_file(filename) for filename in filenames])/len(filenames)}"
    )
    print("-----------\n")
...
def compute_predictions(year, month_num, weights_function):
    ...
    if len(filenames) == 0:
        ...
        return None
    category_expenses_for_files = [
        category_expenses_for_file(filename) for filename in filenames
    ]
    predictions = {}
    for category in CATEGORIES:
        category_expenses = [
            category_expense.get(category, 0)
            for category_expense in category_expenses_for_files
        ]
        predictions[category] = HARDCODED_PREDICTIONS.get(
            category,
            weighted_avg(category_expenses, weights_function(len(category_expenses))),
        )
    return predictions
...
def main():
    ...
    final_predictions = compute_predictions(tomorrow.year, tomorrow.month, weights_func)
